...
function render() {
  ...
  document.querySelector("#app").innerHTML = `
    <header><p class="eyebrow">LOCAL-FIRST EXPENSE SHARING</p><h1>Div It</h1><p id="notice" aria-live="polite"></p></header>
    <section class="card"><label>Group name <input id="group-name" value="${escape(state.name)}"></label><label>Currency <select id="currency"><option ${state.currency === "USD" ? "selected" : ""}>USD</option><option ${state.currency === "INR" ? "selected" : ""}>INR</option><option ${state.currency === "EUR" ? "selected" : ""}>EUR</option><option ${state.currency === "GBP" ? "selected" : ""}>GBP</option></select></label><button id="save-group">Save group</button></section>
    <div class="grid">
      <section class="card"><h2>People</h2><form id="person-form"><input id="person-name" placeholder="Name" required><button>Add</button></form><ul class="people">${state.people.map((entry) => `<li>${escape(entry.name)} <span>${money(balanceMap[entry.id])}</span></li>`).join("") || "<li>Add people to begin.</li>"}</ul></section>
      <section class="card"><h2>Add expense</h2><form id="expense-form"><label>Description <input id="description" placeholder="Dinner" required></label><label>Amount <input id="amount" inputmode="decimal" min="0.01" step="0.01" required></label><label>Paid by <select id="payer" required>${state.people.map((entry) => `<option value="${entry.id}">${escape(entry.name)}</option>`).join("")}</select></label><fieldset id="participants"><legend>Split with</legend>${state.people.map((entry) => `<label><input type="checkbox" value="${entry.id}" checked> ${escape(entry.name)}</label>`).join("") || "Add people first."}</fieldset><label>Split type <select id="split-type"><option value="equal">Equal</option><option value="exact">Exact amounts</option></select></label><div id="exact-splits" hidden></div><button ${state.people.length ? "" : "disabled"}>Add expense</button></form></section>
    </div>
    ...
    <footer><button id="export-json">Export backup</button><label class="import">Import backup <input id="import-json" type="file" accept="application/json"></label></footer>`;
  bindEvents();
}
...
function bindEvents() {
  document.querySelector("#save-group").onclick = async () => { state.name = document.querySelector("#group-name").value.trim() || "My group"; state.currency = document.querySelector("#currency").value; await saveState(); render(); };
  document.querySelector("#person-form").onsubmit = async (event) => { event.preventDefault(); const name = document.querySelector("#person-name").value.trim(); if (!name) return; state.people.push({ id: crypto.randomUUID(), name }); await saveState(); render(); };
  ...
  document.querySelector("#expense-form").onsubmit = async (event) => {
    ...
    try {
      ...
      state.events.push(makeExpense({ description: document.querySelector("#description").value, amount, payerId: document.querySelector("#payer").value, splits }));
      await saveState(); render(); notice("Expense saved locally.");
